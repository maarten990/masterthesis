...
Whereas the Dirichlet distribution samples from a space of discrete
probability \emph{mass} functions (defined over a finite set of values), the
Dirichlet process samples from a space of continuous probablity \emph{density}
functions (defined over an infinite set of values).\citep{dirichlet}. The
probability $w_k$ of the sample being drawn from the $k$'th distribution is
given by
\begin{equation}
